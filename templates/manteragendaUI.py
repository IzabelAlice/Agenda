...
class ManterAgendaUI:
  ...
  def listar():
    ...
    #       if servico.get_id_servico() == servico.get_id():
    #         servico_descricao = servico.get_descricao()
        
    #     dados.append([id, data, confirmado, nome_cliente, servico_descricao])
    # df = pd.DataFrame(dados, columns=["Id", "Data", "Confirmado", "Nome do Cliente", "Descrição do Serviço"])
    # st.dataframe(df)
